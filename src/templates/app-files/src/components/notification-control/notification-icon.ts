export const ComponentsNotificationControlNotificationIconJsx = `import {
  MessageCircle,
  Heart,
  AtSign,
  UserPlus,
...
const getIconConfig = (isDark = false) => {
  ...
  return Object.entries(baseConfig).reduce((acc, [key, value]) => {
    acc[key] = {
      Icon: value.Icon,
...
function NotificationIcon({ type, style = {}, isDarkTheme }) {
  ...
  return (
    <div
      className="flex flex-shrink-0 items-center justify-center rounded-full p-2"
      style={{
        backgroundColor,
        ...style,
      }}
