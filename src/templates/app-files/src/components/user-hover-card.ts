export const ComponentsUserHoverCardJsx = `import { useState } from 'react';
...
export default function UserHoverCard({ userId, children }) {
  ...
  return (
    <HoverCard openDelay={500} closeDelay={200}>
      ...
      <HoverCardContent
        className="z-50 w-80 rounded-xl border border-gray-200 bg-white p-4 shadow-xl"
        align="start"
      >
        {isLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <Link
                to={'/u/' + user.id}
                className="truncate pb-1 text-base font-bold text-gray-900 hover:underline"
              >
                @{user.username}
              </Link>
            </div>
          ...
          <div className="flex items-center justify-center py-4">
            <p className="text-sm text-gray-500">Unable to load user profile</p>
          </div>
        )}
      </HoverCardContent>
    </HoverCard>
