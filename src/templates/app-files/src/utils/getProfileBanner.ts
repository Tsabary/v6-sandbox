export const UtilsGetProfileBannerJs = `// Dynamic colorful profile banner generator using user ID as seed
  ...
  const patterns = [
    // Geometric triangles
    {
      ...
      style: (
        ...
        selectedPalette.colors[2] +
        '40 40px),' +
        'repeating-linear-gradient(-45deg, transparent, transparent 25px, ' +
        selectedPalette.colors[3] +
        '30 25px, ' +
        selectedPalette.colors[3] +
        '30 50px)'
      ).replace(/\\s+/g, ' '),
    ...
    // Organic blob shapes
    {
      type: 'organic-blobs',
      style: (
        'radial-gradient(ellipse 80% 50% at ' +
        (Math.floor(random2 * 40) + 20) +
        '% ' +
        ...
        (Math.floor(random4 * 40) + 40) +
        '% ' +
        (Math.floor(random5 * 30) + 20) +
        '%, ' +
        selectedPalette.colors[1] +
        ' 0%, transparent 50%),' +
        'radial-gradient(ellipse 90% 40% at ' +
        (Math.floor(random6 * 30) + 60) +
        '% ' +
    ...
    // Diagonal stripes party
    {
      ...
      style: (
        ...
        selectedPalette.colors[1] +
        ' 30px),' +
        'repeating-linear-gradient(120deg, transparent 0px, transparent 10px, ' +
        selectedPalette.colors[2] +
        '60 10px, ' +
        selectedPalette.colors[2] +
        '60 20px),' +
    ...
    // Color blocks mosaic
    {
      ...
      style: (
        ...
        '70 25%, ' +
        selectedPalette.colors[3] +
        '70 75%, transparent 75%, transparent 100%)'
      ).replace(/\\s+/g, ' '),
    },
    // Wavy ribbons
    {
